    ...
    <div className='p-6 space-y-4 text-white'>
      <div className='rounded-xl border border-white/10 bg-slate-900 p-4'>
        ...
        <div className='grid grid-cols-1 md:grid-cols-4 gap-2'>
          <input className='px-3 py-2 rounded bg-black/30 border border-white/10' placeholder='Buscar por nome/matrícula' value={q} onChange={e=>{setQ(e.target.value); setPage(1)}}/>
          <select className='px-3 py-2 rounded bg-black/30 border border-white/10' value={regional} onChange={e=>{setRegional(e.target.value); setPage(1)}}>
            <option value=''>Todas as Regionais</option>
            {opts.regionais.map(r=> <option key={r} value={r}>{r}</option>)}
          </select>
          <select className='px-3 py-2 rounded bg-black/30 border border-white/10' value={unidade} onChange={e=>{setUnidade(e.target.value); setPage(1)}}>
