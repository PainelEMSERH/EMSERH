...
type Opts = {
  ...
  unidades: { id: string; nome: string; regionalId: string }[];
};
...
async function fetchJSON<T>(u: string, init?: RequestInit): Promise<T> {
...
export default function Page() {
  ...
  // Filtros principais
  const [regional, setRegional] = useState(''); // guarda o ID da Regional
  const [unidade, setUnidade] = useState('');   // guarda o ID da Unidade
  const [q, setQ] = useState('');

  // Opções de Regionais/Unidades
  ...
  // Itens disponíveis (Item table)
  const [itemOptions, setItemOptions] = useState<ItemOption[]>([]);

  // Catálogo SESMT (planilha)
  ...
  // Carrega opções de Regionais/Unidades
  useEffect(() => {
    fetchJSON<Opts>('/api/estoque/options')
      .then(setOpts)
      .catch(() => {});
  }, []);
  ...
  const unidadesFiltradas = useMemo(
    () => opts.unidades.filter((u) => !regional || u.regionalId === regional),
    [opts, regional],
  );
  ...
  // Busca catálogo SESMT a partir da planilha
  useEffect(() => {
    if (!catQuery.trim()) {
      setCatOptions([]);
      return;
  ...
  // Carrega visão de estoque
  useEffect(() => {
    let mounted = true;
    const url = `/api/estoque/list?regionalId=${encodeURIComponent(
      regional,
    )}&unidadeId=${encodeURIComponent(unidade)}&q=${encodeURIComponent(q)}&page=${page}&size=${size}`;
    fetchJSON<{ rows: Row[]; total: number }>(url)
      .then((d) => {
        if (!mounted) return;
        setRows(d.rows || []);
        setTotal(d.total || 0);
  ...
  useEffect(() => {
    ...
    fetchJSON<{ rows: Mov[]; total: number }>(url)
      .then((d) => {
        if (!mounted) return;
        setMovs(d.rows || []);
  ...
  useEffect(() => {
    ...
    const url = `/api/estoque/pedidos?regionalId=${encodeURIComponent(
      regional,
    )}&unidadeId=${encodeURIComponent(unidade)}&page=${pedPage}&size=25`;
    fetchJSON<{ rows: Pedido[]; total: number }>(url)
      .then((d) => {
        if (!mounted) return;
        setPeds(d.rows || []);
        setPedTotal(d.total || 0);
      })
      .catch(() => {});
    return () => {
      mounted = false;
  ...
  async function criarMov() {
    const unidadeId = novoUnidadeId || unidade;
    if (!unidadeId || !novoItemId || !novoQtd) return;
    const body: any = {
      unidadeId,
      itemId: novoItemId,
      tipo: novoTipo,
      quantidade: novoQtd,
      destino: novoDestino || null,
    ...
    setNovoObs('');
    setNovoData('');
    // recarrega visão e movimentações
    const urlEstoque = `/api/estoque/list?regionalId=${encodeURIComponent(
      regional,
    )}&unidadeId=${encodeURIComponent(unidade)}&q=${encodeURIComponent(q)}&page=${page}&size=${size}`;
    const dEst = await fetchJSON<{ rows: Row[]; total: number }>(urlEstoque);
    setRows(dEst.rows || []);
    setTotal(dEst.total || 0);
    const urlMov = `/api/estoque/mov?regionalId=${encodeURIComponent(
      regional,
    )}&unidadeId=${encodeURIComponent(unidade)}&page=${movPage}&size=25`;
    const dMov = await fetchJSON<{ rows: Mov[]; total: number }>(urlMov);
    setMovs(dMov.rows || []);
    setMovTotal(dMov.total || 0);
  }

  async function criarPedido() {
    if (!unidade || pedItens.length === 0) return;
    const body: any = {
      regionalId: regional || null,
      unidadeId: unidade || null,
      previstoEm: pedPrevisto || null,
    ...
    const d = await fetchJSON<{ rows: Pedido[]; total: number }>(url);
    setPeds(d.rows || []);
    setPedTotal(d.total || 0);
  }
    ...
    <div className="p-6 space-y-4">
      <div className="rounded-xl border border-border bg-panel p-4">
        ...
        <p className="text-sm text-muted mb-3">
          Controle completo do estoque físico por Regional/Unidade: saldo, movimentações e pedidos de reposição.
        </p>
        <div className="grid grid-cols-1 md:grid-cols-4 gap-2 text-sm">
          <input
            className="px-3 py-2 rounded bg-card border border-border"
        ...
        <div className="mt-4 flex gap-2 text-xs">
          <button
            ...
            className={`px-3 py-1 rounded border ${tab === 'visao' ? 'bg-white/10' : ''}`}
          >
            Visão Geral
          </button>
          <button
            onClick={() => setTab('mov')}
            className={`px-3 py-1 rounded border ${tab === 'mov' ? 'bg-white/10' : ''}`}
          >
            Movimentações
          </button>
          <button
            onClick={() => setTab('ped')}
            className={`px-3 py-1 rounded border ${tab === 'ped' ? 'bg-white/10' : ''}`}
          >
      ...
      {tab === 'visao' && (
        <div className="rounded-xl border border-border bg-panel">
            ...
            <table className="min-w-full text-sm">
              ...
              <tbody>
                {rows.map((r) => {
                  ...
                  let badge = 'OK';
                  let badgeClass =
                    'inline-flex items-center rounded-full px-2 py-0.5 text-[11px] bg-emerald-500/10 text-emerald-300';
                  if (zero) {
                    badge = 'Zerado';
                    badgeClass =
                      'inline-flex items-center rounded-full px-2 py-0.5 text-[11px] bg-red-500/10 text-red-300';
                  } else if (baixo) {
                    badge = 'Abaixo do mínimo';
                    badgeClass =
                      'inline-flex items-center rounded-full px-2 py-0.5 text-[11px] bg-amber-500/10 text-amber-300';
                  } else if (semMinimo) {
                    badge = 'Sem mínimo';
                    badgeClass =
                      'inline-flex items-center rounded-full px-2 py-0.5 text-[11px] bg-slate-500/10 text-slate-200';
                  }
                  return (
                    <tr
                      key={r.id}
                ...
                {rows.length === 0 && (
                  <tr>
                    <td colSpan={8} className="px-3 py-6 text-center text-muted">
                      Nenhum registro de estoque para os filtros selecionados.
                    </td>
                  </tr>
                )}
              </tbody>
          ...
          {/* Cadastro rápido de novo item + catálogo SESMT */}
          <div className="border-t border-border px-4 py-4 text-xs bg-card/20 flex flex-col gap-3">
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-2">
              <div>
                <div className="font-semibold text-sm">Cadastrar novo item de estoque</div>
                <div className="text-muted">
                  Utilize o catálogo SESMT (Cahosp) para buscar EPI / placas / itens autorizados. O estoque inicial será
                  lançado para a unidade selecionada.
                </div>
              </div>
            ...
            {catOptions.length > 0 && (
              <div className="max-h-52 overflow-y-auto rounded border border-border bg-card text-[11px] mt-2">
                {catOptions.map((c, idx) => (
                  ...
                  >
                    ...
                    <div className="text-muted">
                      ...
                      {c.categoria_site ? ` · Categoria: ${c.categoria_site}` : ''}
                      {c.unidade_site ? ` · Unidade: ${c.unidade_site}` : ''}
              ...
              <button
                ...
                onClick={async () => {
                  try {
                    ...
                    setRows(d.rows || []);
                    setTotal(d.total || 0);
                    const itemsResp = await fetchJSON<{ items: ItemOption[] }>('/api/estoque/items');
        ...
        <div className="space-y-4">
          <div className="rounded-xl border border-border bg-panel p-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-6 gap-2">
              ...
              <select
                className="px-3 py-2 rounded bg-card border border-border text-xs"
                value={novoUnidadeId}
                onChange={(e) => setNovoUnidadeId(e.target.value)}
              >
            ...
            {itensCat.length === 0 && (
              <div className="mt-2 text-[11px] text-amber-300">
                Nenhum item cadastrado ainda. Use a aba &quot;Visão Geral&quot; &gt; &quot;Cadastrar novo item&quot; para
                trazer itens do catálogo SESMT para o estoque.
              </div>
            )}
            <div className="mt-3 flex justify-end">
              <button
                ...
                onClick={criarMov}
                disabled={!novoUnidadeId || !novoItemId || !novoQtd}
              >
                Salvar movimentação
              </button>
            </div>
          ...
          <div className="rounded-xl border border-border bg-panel">
            <div className="px-4 py-3 border-b border-border flex items-center justify-between text-xs">
              <div className="font-semibold">Histórico de movimentações</div>
              <div className="text-muted">
                Total: {movTotal} • Página {movPage}
              </div>
            </div>
            <div className="overflow-x-auto">
                ...
                <tbody>
                  {movs.map((m) => (
                    <tr key={m.id} className="border-t border-border hover:bg-white/5">
                      ...
                      <td className="px-3 py-2">{m.regional}</td>
        ...
        <div className="space-y-4">
          <div className="rounded-xl border border-border bg-panel p-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-5 gap-2">
              ...
              />
              <input
                className="px-3 py-2 rounded bg-card border border-border text-xs md:col-span-3"
                placeholder="Observação"
                value={pedObs}
              ...
              <div className="grid grid-cols-1 md:grid-cols-6 gap-2 items-center">
                <select
                  ...
                  onChange={(e) => {
                    ...
                    if (!val) return;
                    setPedItens((p) => [...p, { itemId: val, quantidade: 1 }]);
                    e.target.value = '';
                  }}
                >
                ...
                <div className="col-span-5 text-xs text-muted">
                  {pedItens.map((it, idx) => (
                    <span
                      key={idx}
                      className="inline-flex items-center gap-1 px-2 py-1 border border-border rounded mr-2 mb-2"
                    >
                      <input
                        type="number"
                        ...
                        onChange={(e) => {
                          ...
                          setPedItens((arr) =>
                            arr.map((x, i) => (i === idx ? { ...x, quantidade: v } : x)),
                          );
                        }}
                      />
                      <button
                        onClick={() =>
                          setPedItens((arr) => arr.filter((_, i) => i !== idx))
                        }
                      >
                        ✕
                      </button>
                  ...
                  {pedItens.length === 0 && (
                    <span className="text-[11px]">
                      Nenhum item adicionado. Use o seletor ao lado para incluir itens do estoque.
                    </span>
                  )}
                </div>
              </div>
            ...
            <div className="mt-3 flex justify-end">
              <button
                className="px-3 py-2 border rounded text-xs disabled:opacity-50"
                onClick={criarPedido}
                disabled={!unidade || pedItens.length === 0}
              >
                Salvar pedido
              </button>
            </div>
          ...
          <div className="rounded-xl border border-border bg-panel">
            <div className="px-4 py-3 border-b border-border flex items-center justify-between text-xs">
              <div className="font-semibold">Pedidos de reposição</div>
              <div className="text-muted">
                Total: {pedTotal} • Página {pedPage}
              </div>
            </div>
            <div className="overflow-x-auto">
              <table className="min-w-full text-xs">
                ...
                <tbody>
                  {peds.map((p) => (
                    <tr key={p.id} className="border-t border-border hover:bg-white/5">
                      ...
                      <td className="px-3 py-2">{p.regional}</td>
                      <td className="px-3 py-2">{p.unidade}</td>
                      <td className="px-3 py-2">
                        {p.previstoEm ? new Date(p.previstoEm).toLocaleDateString('pt-BR') : ''}
                      </td>
                      <td className="px-3 py-2">
                        {p.recebidoEm ? new Date(p.recebidoEm).toLocaleDateString('pt-BR') : ''}
                      </td>
                      <td className="px-3 py-2 text-right">{p.qtd_solicitada}</td>
                      <td className="px-3 py-2 text-right">{p.qtd_recebida}</td>
                      <td className="px-3 py-2">
                        <span className="inline-flex items-center rounded-full px-2 py-0.5 text-[11px] border border-border">
                          {p.status}
                        </span>
                      </td>
                    </tr>
                  ...
                  {peds.length === 0 && (
                    <tr>
                      <td colSpan={8} className="px-3 py-6 text-center text-muted">
                        Nenhum pedido registrado para os filtros selecionados.
                      </td>
                    </tr>
                  )}
                </tbody>
            ...
            <div className="flex items-center justify-between p-3 text-xs text-muted">
              ...
              <div className="flex items-center gap-2">
                <button
                  className="px-2 py-1 border border-border rounded disabled:opacity-40"
                  onClick={() => setPedPage((p) => Math.max(1, p - 1))}
                  disabled={pedPage === 1}
                >
                  Anterior
                </button>
                ...
                <button
                  className="px-2 py-1 border border-border rounded disabled:opacity-40"
                  onClick={() => setPedPage((p) => p + 1)}
                  disabled={peds.length < 25}
                >
                  Próxima
                </button>
              </div>
