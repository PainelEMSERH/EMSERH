  ...
  return (
    <div className="p-6 space-y-4">
      <div className="rounded-xl border border-white/10 bg-[#0f172a] p-4">
        ...
        <p className="text-sm text-white/60 mb-3">Baseado em <code>stg_epi_map</code> (quantidade por item e função).</p>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
          <input className="px-3 py-2 rounded bg-black/20 border border-white/10" placeholder="Filtrar por função ou item" value={q} onChange={e=>{setQ(e.target.value); setPage(1)}}/>
          <input className="px-3 py-2 rounded bg-black/20 border border-white/10" placeholder="Filtrar por unidade (nome_site)" value={unidade} onChange={e=>{setUnidade(e.target.value); setPage(1)}}/>
        </div>
      </div>

      {porFuncao.map(([func, itens]) => (
        <div key={func} className="rounded-xl border border-white/10 bg-[#0f172a]">
          <div className="px-4 py-3 border-b border-white/10 font-semibold">{func}</div>
      ...
      {rows.length===0 && (
        <div className="rounded-xl border border-white/10 bg-[#0f172a] p-6 text-center text-white/60">Nenhum registro</div>
      )}
    </div>
  );
}
